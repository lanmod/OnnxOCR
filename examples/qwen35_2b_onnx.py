...
def download_from_modelscope(repo_id: str, target_dir: Path, variant: str, repo_subdir: str) -> int:
    ...
    cache_dir = Path(snapshot_download(repo_id))
    source_dir = cache_dir / repo_subdir if repo_subdir else cache_dir
    if not source_dir.exists():
        print(f"ModelScope repository subdirectory not found: {source_dir}", file=sys.stderr)
        return 1
